...
UTILS_PATH = PROJECT_DIR / "src" / "utils.py"
MAX_ROWS_IN_TEST = 3
...
def load_utils_module():
    return load_module("utils", UTILS_PATH)
...
class TestTrainTokenizer:
    ...
    def test_filtered_text_iterable_reads_only_first_rows_and_filters_min_text_length(
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            byte_min_text = "あ" * ((utils.MIN_TEXT_LENGTH // 3) + 1)
            rows = [
                {"text": "a" * utils.MIN_TEXT_LENGTH},
                {"text": "too short"},
                {"text": byte_min_text},
                {"text": "c" * utils.MIN_TEXT_LENGTH},
            ]
        ...
        assert len(byte_min_text) < utils.MIN_TEXT_LENGTH
    ...
    def test_filtered_text_iterable_keeps_long_utf8_text_when_max_is_unset(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            valid_text = "a" * utils.MIN_TEXT_LENGTH
    ...
    def test_filtered_text_iterable_normalizes_multiline_text_to_one_line(self):
        ...
        assert "\n" not in texts[0]
        assert len(texts[0]) >= utils.MIN_TEXT_LENGTH
    ...
    def test_train_tokenizer_supplies_configured_shard_regex(self, monkeypatch):
        train_tokenizer = load_script()
        utils = load_utils_module()
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            input_path = root / "sample-0000.jsonl.zst"
            expected_text = "a" * utils.MIN_TEXT_LENGTH
    ...
    def test_iter_zstd_jsonl_lines_reads_zst_files_without_subprocess(
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            lines = list(utils.iter_zstd_jsonl_lines(path, max_rows_per_file=1))
