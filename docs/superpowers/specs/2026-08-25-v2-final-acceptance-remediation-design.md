...
remediation, Task 6.4, Part 2, and umbrella refactor are complete, including
final acceptance, as of 2026-09-07. The final production correction is
...
legacy bridge string remains; the CLI/cutover scans pass; help lists
...
The completed checkpoint reader retains shared-lock protection on local APFS
and uses an explicit reduced-guarantee read-only mode only for proven local
non-APFS storage or narrow sidecar permission/read-only failures. That mode
does not promise concurrent-writer or pruning exclusion; other lock and
filesystem failures remain fail-closed.
...
but left a test-only follow-up. The September 7 correction at `73df250` and
its zero-finding scoped review closed that follow-up as recorded above.
